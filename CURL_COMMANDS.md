...
### 7. Actualizar Item en el Carrito

**Descripción:** Reemplaza un producto existente en el carrito por otro (ej. cambio de modelo o actualización de precio/cantidad).

**Endpoint:** `POST /carrito/actualizar-item`

**Body esperado:**
- `usuario_id` (string): ID del usuario
- `item_id_antiguo` (string): ID del producto a reemplazar
- `nuevo_item`: Objeto con los detalles del nuevo producto

```bash
curl -X POST "http://localhost:8000/carrito/actualizar-item" \
  -H "Content-Type: application/json" \
  -d '{
    "usuario_id": "mi-usuario-123",
    "item_id_antiguo": "producto-1",
    "nuevo_item": {
      "item_id": "producto-1-v2",
      "nombre": "Laptop Pro",
      "precio": 1299.99,
      "cantidad": 1
    }
...
### 10. Aprobar Orden (Acción Humana)

**Descripción:** Simula la acción de un operador humano aprobando la orden. En el patrón *Human in the Loop*, el workflow se habría detenido previamente esperando esta confirmación. Este endpoint envía una **Señal** al workflow para desbloquearlo.

**Endpoint:** `POST /admin/aprobar-orden`

```bash
curl -X POST "http://localhost:8000/admin/aprobar-orden" \
  -H "Content-Type: application/json" \
  -d '{"usuario_id": "mi-usuario-123", "decision": "APROBADO"}'
```
```
